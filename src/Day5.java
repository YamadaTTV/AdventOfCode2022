...
public class Day5 {
    ...
    public static void part2(List<String> list){
        ...
        Stack<Character> to = new Stack<>();
        try{
            list = Files.readAllLines(Paths.get("src\\resources\\input5.txt"));
        } catch (IOException e){
            e.printStackTrace();
        }
        for(String s: list){
            if(s.isEmpty() || s.charAt(1) == '1'){
                continue;
            }
            if(!s.startsWith("move")) {
                if (s.charAt(1) != ' ') {
                    stack1.add(stack1.size(), s.charAt(1));
                }
                if (s.charAt(5) != ' ') {
                    stack2.add(stack2.size(), s.charAt(5));
                }
                if (s.charAt(9) != ' ') {
                    stack3.add(stack3.size(), s.charAt(9));
                }
                if (s.charAt(13) != ' ') {
                    stack4.add(stack4.size(), s.charAt(13));
                }
                if (s.charAt(17) != ' ') {
                    stack5.add(stack5.size(), s.charAt(17));
                }
                if (s.charAt(21) != ' ') {
                    stack6.add(stack6.size(), s.charAt(21));
                }
                if (s.charAt(25) != ' ') {
                    stack7.add(stack7.size(), s.charAt(25));
                }
                if (s.charAt(29) != ' ') {
                    stack8.add(stack8.size(), s.charAt(29));
                }
                if (s.charAt(33) != ' ') {
                    stack9.add(stack9.size(), s.charAt(33));
                }
            } else {
                // Movement Control
                String[] movementControl = s.split(" ");
                switch (movementControl[3]) {
                    case "1" -> from = stack1;
                    case "2" -> from = stack2;
                    case "3" -> from = stack3;
                ...
                for(int i = 0; i < Integer.parseInt(movementControl[1]); i++){
                    to.add(i, from.get(0));
                    from.remove(0);
                }
            }
        ...
        System.out.println("Answer: The element of top of each stack is: " + answer);
    }
